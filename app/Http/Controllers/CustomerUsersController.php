...
class CustomerUsersController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        */
        ...
        return redirect(route('customers.edit', $customer_id) . $section)
            ->with('success', l('This record has been successfully created &#58&#58 (:id) ',
                                ['id' => $customeruser->id], 'layouts') . $customeruser->getFullName());
    }
    ...
    public function update(Request $request, CustomerUser $customeruser)
    {
        ...
        if (isset($vrules['email'])) {
            $vrules['email'] .= ',' . $customeruser->id . ',id';
        }  // Unique
        ...
        if ($request->input('password') != '') {
            ...
            $validator = Validator::make($request->all(), array_except($vrules, ['password']));
            if (!$validator->passes()) {
                return response()->json(['error' => $validator->errors()->all()]);
            }
