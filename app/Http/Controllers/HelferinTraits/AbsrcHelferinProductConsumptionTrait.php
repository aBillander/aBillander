...
trait AbsrcHelferinProductConsumptionTrait
{
    ...
    public function reportConsumption(Request $request)
    {
        ...
        if ( isset(Auth::user()->sales_rep_id) && Auth::user()->sales_rep_id )
        ...
        foreach ($products as $product) {
        	...
        	$product->consumption = CustomerShippingSlipLine::
	                    ...
	                    ->whereHas('document', function ($query) use ($sales_rep_id, $customer_id, $date_from, $date_to, $document_reference_date) {
                                ...
                                $query->whereHas('customer', function ($query) use ($sales_rep_id) {
                                    if ( $sales_rep_id > 0 )
                                        ...
                                        $query->where('sales_rep_id', -1);  // Prevent unauthorized
                                });
