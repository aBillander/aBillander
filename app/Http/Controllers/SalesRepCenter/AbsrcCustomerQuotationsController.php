...
use App\Models\CustomerQuotationLineTax as DocumentLineTax;
...
class AbsrcCustomerQuotationsController extends BillableController
{
   use DateFormFormatterTrait;

   protected $salesrep, $salesrep_user, $customerQuotation, $customerQuotationLine;
   ...
   public function __construct(SalesRep $salesrep, SalesRepUser $salesrep_user, Customer $customer, Document $document, DocumentLine $document_line)
   {
        ...
        $this->model = 'CustomerQuotation';	// \Str::singular($this->getParentClass());       // CustomerShippingSlip
        $this->model_snake_case = 'quotation';	// $this->getParentModelSnakeCase(); // customer_shipping_slip
        $this->model_path = 'absrc.quotations';	// $this->getParentClassLowerCase();       // customershippingslips
        $this->model_url = 'absrc/quotations';
        $this->view_path = 'absrc.quotations';	// $this->getParentClassSnakeCase();            // customer_shipping_slips
...
//        $doc_id = $seq->getNextDocumentId();
        ...
        $data = [
            ...
            'url' => route('customerquotations.edit', [$document->id]), 
            'due_date' => null, 
            'completed' => 0, 
...
if (0) {
        ...
        try {
            ...
            $template_vars = array(
//                'company'       => $company,
                'document_num'   => $customerQuotation->document_reference,
                'document_date'  => abi_date_short($customerQuotation->document_date),
                'document_total' => $customerQuotation->as_money('total_tax_excl'),
//                'custom_body'   => $request->input('email_body'),
            ...
            $send = Mail::send('emails.'.Context::getContext()->language->iso_code.'.absrc.new_customer_quotation', $template_vars, function($message) use ($data)
            {
            ...
            return redirect()->route('absrc.quotations.index')
	...
	public function show($id)
	{
		...
		return view('absrc.quotations.show', compact('quotation'));
	}
	 ...
	 * @return Response
	 */
	public function update(Request $request, Document $quotation)
	{
        // abi_r($quotation);die();
...
*/
    ...
    public function deleteDocumentLine($line_id)
    {
        ...
        $document_line = $this->document_line
                        ->findOrFail($line_id);
/*
        $document = $this->customerQuotation
                        ->findOrFail($quotation_line->customer_quotation_id);
*/
    ...
    public function duplicateOrder($id)
    {
        ...
        foreach ($quotation->lines as $quotationline) {
        	# code...
        	$line = $cart->addLine($quotationline->product_id, $quotationline->combination_id, $quotationline->quantity);
        }

        return redirect()->route('absrc.cart')
                ->with('success', l('This record has been successfully created &#58&#58 (:id) ', ['id' => $quotation->id], 'layouts'));
    }
	...
	public function showPdf($id, Request $request)
	{
        ...
        if ( !$t )
        	return redirect()->route('absrc.quotations.show', $id)
