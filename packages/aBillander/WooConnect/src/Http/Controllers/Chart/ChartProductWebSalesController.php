...
namespace aBillander\WooConnect\Http\Controllers\Chart;
...
//	}
	...
	function getMonthlyProductWebSales(Request $request) {
		...
		return view('woo_connect::charts.woo_product_sales', compact('has_data', 'labels'));
    }
...
//				$date = new \DateTime( $unformatted_date->date );
				...
				$month_name = l('month.'.$month_no, 'chart');	//$date->format( 'M' );
	...
	function getMonthlyPostCount( $month, $year ) 
	{
		...
		$monthly_order_count = $class::
                        ...
                        ->whereHas('document', function ($query) use ( $product_date_from, $product_date_to, $document_reference_date, $is_invoiceable_flag) {
                        		...
                        		// Web Documents only
                                $query->where('created_via', 'webshop');
...
//								->sum('quantity')
	...
	// Entry point
	// Magic happens here:
	function getMonthlyProductWebSalesData(Request $request) {
        ...
        $model = $this->model = $request->input('product_sales_model', 'CustomerOrder');
