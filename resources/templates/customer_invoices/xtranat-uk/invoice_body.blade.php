		...
		<td class="order-data">
			...
			<table>
				...
				<tr class="order-number">
					...
					<th style="width: 45%; font-weight: bold;font-size: 16pt;vertical-align: baseline !important;">INVOICE:</th>

					<td style="width: 55%; font-weight: bold;font-size: 11pt;vertical-align: baseline !important;">{!! $document->document_reference ?? 'DRAFT' !!}</td>
...
@else
	@if ( optional($line->product)->ecotax )
		...
		@include('templates::customer_invoices.xtranat-uk.line_rae')
...
@endif
@if ( AbiConfiguration::isTrue('PRINT_LOT_NUMBER_ON_DOCUMENTS') && $line->lot_references && $line->product->lot_tracking )
@php
	$names = [];
...
@elseif ( $document->customer->vat_regime == 2 )
		...
		<tr class="3655">
			...
			<td class="description" colspan="7">
				<span>
					<span class="item-name"><strong> Exempt export transaction under art.21 of Spanish VAT law 37/1992. </strong></span>
					<span class="item-combination-options"></span>
				</span>
			</td>
		</tr>
...
@include('templates::customer_invoices.xtranat-uk.totals')
...
@if( !optional($document->paymentmethod)->payment_is_cash || 1 )
						...
						<h3>Bank account: {{ optional(AbiContext::getContext()->company->bankaccount)->iban_presenter(true) }}</h3>
...
@endif
								<tr class="total grand-total">
									...
									<th class="description" style="background-color: #f5f5f5;"><span>Total</span></th>
...
@include('templates::customer_invoices.xtranat-uk.payments')
...
$GLOBALS['var'] = 'Invoice number:   ' . ($document->document_reference ?: 'BORRADOR');
...
<script type="text/php">
    ...
    if ( isset($pdf) ) {
        ...
        {
			// $pdf->page_text(($pdf->get_width() - 82), ($pdf->get_height() - 26.89), "Page {PAGE_NUM} de {PAGE_COUNT}", null, 9);

        	$pdf->page_text(($pdf->get_width() - 61), ($pdf->get_height() - 26.89 - 43), "Page", null, 9);
