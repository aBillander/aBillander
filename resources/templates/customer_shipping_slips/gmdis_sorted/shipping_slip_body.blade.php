...
@php
// Filters
$comments = $document->documentlines->filter(function ($value, $key) {
    return $value->line_type == 'comment';
})->sortBy('line_sort_order');

$products = $document->documentlines->filter(function ($value, $key) {
    return $value->line_type == 'product';
})->sortBy(function ($line, $key) {
    return $line->product->location;
});

$services = $document->documentlines->filter(function ($value, $key) {
    return $value->line_type == 'service';
})->sortBy('line_sort_order');

$shippings = $document->documentlines->filter(function ($value, $key) {
    return $value->line_type == 'shipping';
})->sortBy('line_sort_order');

$lines = $comments->merge($products)->merge($services)->merge($shippings);
...
@else
	@if ( optional($line->product)->ecotax )
		...
		@include('templates::customer_shipping_slips.gmdis_sorted.line_rae')
...
@include('templates::customer_shipping_slips.gmdis_sorted.totals')
