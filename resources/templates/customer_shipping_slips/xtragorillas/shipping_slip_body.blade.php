...
<table class="head container" style="margin-top: -0.7cm !important">
	...
	<tr>
        ...
        @if ($img = AbiContext::getContext()->company->company_logo)
            ...
            <img class="img-rounded" height="{{ '60' }}" src="{{ URL::to( \App\Models\Company::imagesPath() . $img ) }}">
...
@php
	...
	if ( $document->leftOrders()->count() == 1 )
	{
		//
		$order = $document->leftOrders()->first();
		$reference_customer = $order->reference_customer;
	}
@endphp
					<th><strong>Nº orden cliente:<strong></th>

					<td><strong>{{ $reference_customer }}</strong></td>
...
<table class="order-details" width="100%" style="border: 1px #ccc solid">
	...
	<thead>
		...
		<tr>
			...
			<th class="wrin first-column" width="14%" style="border: 1px #ccc solid">
				<span>WRIN</span>
			</th>
			<th class="sku" width="5%" style="border: 1px #ccc solid">
				<span>Ref.</span>
			</th>
			<th class="description" width="40%" style="border: 1px #ccc solid">
				<span>Producto</span>
			</th>
			<th class="quantity" width="6%" style="border: 1px #ccc solid">
				<span>Uds.</span>
			</th>
			<th class="packages" width="6%" style="border: 1px #ccc solid">
				<span>Cajas</span>
			</th>
			...
			<th class="total xlast-column" width="8%" style="border: 1px #ccc solid">
				<span>Total</span>
			</th>
		</tr>
...
@else
	@if ( optional($line->product)->ecotax )
		...
		@include('templates::customer_shipping_slips.xtragorillas.line_rae')
	
	@else
		<tr class="3655">
			<td class="wrin first-column">
				<span>{{ $line->product->reference_external_wrin }}</span>
			</td>
			<td class="sku">
				<span>{{ $line->reference }}</span>
			</td>
...
@if ( AbiConfiguration::isTrue('ENABLE_LOTS') && ($line->line_type == 'product') && ($line->product->lot_tracking > 0) )
	@include('templates::customer_shipping_slips.xtragorillas.line_block_lots')
@endif
...
@if ( $line->package_measure_unit_id != $line->measure_unit_id )
				<span>{{ $line->as_quantityable($line->quantity / $line->pmu_conversion_rate, 0) }}</span>
@endif
...
@include('templates::customer_shipping_slips.xtragorillas.totals')
...
if ( $PAGE_NUM == 1 )
{
               ...
               $pdf->text(($pdf->get_width() - 150), ($pdf->get_height() - 33.89 - 635.0), $PAGE_NUM." de ".$PAGE_COUNT, null, 9);
}
if ( $PAGE_NUM > 1 )
{
