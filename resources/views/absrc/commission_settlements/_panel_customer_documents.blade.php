...
<table id="lines" class="table table-hover">
	...
	@foreach ($settlement->commissionsettlementlines as $line)
      ...
      <td>{{ $line->commissionable->status_name }}</td>
			<td>
          <a href="{{ URL::to('absrc/customers/' . optional(optional($line->commissionable)->customer)->id . '/edit') }}" title="{{l('Go to', [], 'layouts')}}" target="_blank">{{ $line->commissionable->customer->name_regular or '' }}</a></td>
