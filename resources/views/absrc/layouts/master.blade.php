...
<html lang="{{ AbiContext::getContext()->language->iso_code }}">
    <head>
        ...
        <!-- Styles -->
        <link rel="shortcut icon" href="{{ asset('assets/theme/company_absrc_icon.png') }}" type="image/x-icon">
        ...
        <link href="{{ asset('assets/theme/css/custom-absrc.css') }}" rel="stylesheet" type="text/css"/>
        <link href="{{ asset('assets/font-awesome/css/font-awesome.min.css') }}" rel="stylesheet" type="text/css"/>
        @yield('styles')
    </head>
    <body>
        <div id="app">
            @include('absrc/layouts/nav')
            <div class="container-fluid" style="margin: 10px 0px 10px 0px;"> 
                @include('absrc/layouts/notifications')
                @yield('content')

    {{-- --}}

            @auth('customer')
                @include('absrc/layouts/modal_feedback')
            @endif
