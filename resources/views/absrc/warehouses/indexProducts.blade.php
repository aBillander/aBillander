...
@section('title') {{ l('Warehouse Products') }} @parent @endsection
...
<div class="page-header">
    <div class="pull-right" style="padding-top: 4px;">
        ...
        <a class="btn xbtn-sm btn-grey" href="{{ route('absrc.warehouse.inventory.export', [$warehouse->id] + Request::all() ) }}" title="{{l('Export', [], 'layouts')}}"><i class="fa fa-file-excel-o"></i> {{l('Export', [], 'layouts')}}</a>
    ...
    <h2>
        <a href="{ { URL::to('warehouses') } }" onClick="return false;">{{ l('Warehouse Products') }}</a> <span style="color: #cccccc;">/</span> {{ $warehouse->address->alias }} &nbsp; <a href="javascript:void(0);" class="btn btn-lightblue btn-xs"><span class="badge">{{$products->total()}}</span> {{l('product(s)')}}</a>
    </h2>        
</div>
    ...
    <div class="col-md-12 xcol-md-offset-3">
        <div class="panel panel-info">
            ...
            <div class="panel-body">
                ...
                {!! Form::model(Request::all(), array('route' => array('absrc.warehouse.inventory', $warehouse->id), 'method' => 'GET')) !!}
...
{!! link_to_route('absrc.warehouse.inventory', l('Reset', [], 'layouts'), [$warehouse->id], array('class' => 'btn btn-warning')) !!}
</div>
...
<table id="stockcounts" class="table table-hover">
  ...
  @foreach ($products as $product)
    <tr>
      <td>{{ $product->id }}</td>
            <td><a href="{ { URL::to('products/' . $product->id . '/edit') } }" onClick="return false;" title="{{l('Edit', [], 'layouts')}}" target="_new">{{ $product->reference }}</a></td>
            <td>{{ $product->name }}</td>
