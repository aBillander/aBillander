    ...
    <div class="col-md-12 xcol-md-offset-3">
        <div class="panel panel-info">
            ...
            <div class="panel-body">
                ...
                {!! Form::model(Request::all(), array('route' => 'assemblyorders.index', 'method' => 'GET')) !!}
...
{!! link_to_route('assemblyorders.index', l('Reset', [], 'layouts'), null, array('class' => 'btn btn-warning')) !!}
</div>
...
<div id="div_assemblyorders">
   <div class="table-responsive">

@if ($orders->count())
<table id="assemblyorders" class="table table-hover">
    <thead>
        <tr>
			<th>{{l('ID', [], 'layouts')}}</th>
            <th>{{l('Due Date')}}</th>
	...
	@foreach ($orders as $order)
		<tr>
            <td>{{ $order->id }}</td>
            <td>{{ abi_date_short($order->due_date) }}</td>
            <!-- td>{{ $order->product_reference }}</td -->
            <td>
                [<a href="{{ URL::to('products/' . $order->product->id . '/edit') }}" title="{{l('View Product')}}" target="_blank">{{ $order->product_reference }}</a>] {{ $order->product_name }}
            </td>
            <td>{{ $order->measureunit->quantityable($order->planned_quantity) }}</td>
            <td>{{ $order->measureunit->name }}</td>
