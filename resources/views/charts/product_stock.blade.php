...
@section('title') {{ l('Chart - Aggregated Stock Movements') }} @parent @endsection
...
<div class="page-header">
    ...
    <h2>
        {{ l('Aggregated Stock Movements') }} :: [<a target="_blank" href="{{ route('products.edit', $product->id) }}" title="{{l('Go to', [], 'layouts')}}">{{ $product->reference }}</a>] {{ $product->name }} ({{ $product->measureunit->name }})
    </h2>        
</div>
...
var charts = {
        ...
        ajaxGetPostMonthlyData: function () {
            var urlPath =  "{{ route( 'chart.product.stock.monthly.data', ['product_id' => $product->id] ) }}";
            var request = $.ajax( {
                method: 'GET',
                url: urlPath
        } );
