	...
	@foreach ($contents as $content)
		<tr>
			...
			<td class="text-right">
                ...
                <a class="btn btn-sm btn-blue" href="{{ URL::to('countries/' . $content->id . '/states') }}" title="{{l('Show States')}}"><i class="fa fa-folder-open-o"></i></a>               
                <a class="btn btn-sm btn-warning" href="{{ URL::to('countries/' . $content->id . '/edit') }}" title="{{l('Edit', [], 'layouts')}}"><i class="fa fa-pencil"></i></a>
                <a class="btn btn-sm btn-danger delete-item" data-html="false" data-toggle="modal" 
                		href="{{ URL::to('countries/' . $content->id ) }}" 
                		data-content="{{l('You are going to delete a record. Are you sure?', [], 'layouts')}}" 
                		data-title="{{ l('Help Contents') }} :: ({{$content->id}}) {{ $content->name }}" 
                		onClick="return false;" title="{{l('Delete', [], 'layouts')}}"><i class="fa fa-trash-o"></i></a>
                @else
                <a class="btn btn-warning" href="{{ URL::to('countries/' . $content->id. '/restore' ) }}"><i class="fa fa-reply"></i></a>
                <a class="btn btn-danger" href="{{ URL::to('countries/' . $content->id. '/delete' ) }}"><i class="fa fa-trash-o"></i></a>
                @endif
