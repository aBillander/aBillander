...
@section('title') {{ l('Import - Supplier Price List') }} @parent @endsection
...
{{--
          ...
          <div class="panel-body">
            ...
            <br />
                    <span class="badge" style="background-color: #3a87ad;" title="{{ $supplier->currency->name }}">{{ $supplier->currency->iso_code }}</span> ({{ l(':decimals decimals', ['decimals' => $supplier->currency->decimalPlaces])}} )
                        <br />
                    <span class="label label-success">{{ $supplier->getType() }}</span>
                    @if ($supplier->type != 'price')
                      <span class="label label-default">{{ $supplier->as_percent('amount') }}%</span>
                    @endif
                    @if ( $supplier->price_is_tax_inc )
                        <br />
                        <span class="label label-info">{{ l('Tax Included', [], 'suppliers') }}</span>
                    @endif
            <br />
            <br />
...
--}}
      ...
      <div class="col-lg-8 col-md-8 col-sm-8">
          ...
          @include('imports._panel_supplier_price_list_lines')
