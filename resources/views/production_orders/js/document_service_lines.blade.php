...
//                    $("[data-toggle=popover]").popover();
    	...
    	function editDocumentServiceLine( selector ) {
            ...
            // Load form first
               var panel = $("#document_line_form");
               var url = "{{ route('productionorders.serviceform', ['edit']) }}";
...
//                    $('#line_tax_label').html(result.tax_label);
        ...
        function auto_service_line( selector = "#line_autoservice_name" ) {
            ...
            $( selector ).autocomplete({
                source : "{{ route('productionorders.searchservice') }}?customer_id="+$('#customer_id').val()+"&currency_id="+$('#currency_id').val(),
                minLength : 1,
