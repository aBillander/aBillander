   ...
   <div class="row">
      ...
      <div class="col-lg-10 col-md-10 col-sm-10">
            <div class="panel panel-success" id="panel_production_orders_assemblies">
               <div class="panel-heading">
                  <h3 class="panel-title"><i class="fa fa-cube"></i> &nbsp; {!! l('Production Orders &#58&#58 Assemblies') !!} - <strong>{{ \App\Configuration::get('ASSEMBLY_GROUP_'.$i.'_TAG') }}</strong> [{{ \App\Configuration::get('ASSEMBLY_GROUP_'.$i) }}] - ({{ $sheet->productionorders->where('schedule_sort_order', \App\Configuration::get('ASSEMBLY_GROUP_'.$i))->count() }})</h3>
               </div>
                    @include('production_sheet_production_orders._panel_production_orders_assemblies', ['schedule_sort_order' => \App\Configuration::get('ASSEMBLY_GROUP_'.$i)])
            </div>
      </div>
   </div>
@endfor
