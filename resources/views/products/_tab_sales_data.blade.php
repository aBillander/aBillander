...
@if( AbiConfiguration::isTrue('ENABLE_ECOTAXES') && $product->ecotax)
    {!! l('Prices are exclusive of Ecotax') !!} ( <span class="text-success">{{ $product->as_priceable($product->ecotax->amount) }} {{ AbiContext::getContext()->currency->name }}</span> ).
@endif
    ...
    <table id="aBook" class="table table-hover">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $product->as_percentable( \App\Models\Calculator::discount( $product->price, $line_price, $pricelist->currency ) ) }}</td>
                <td>{{ $product->as_percentable( \App\Models\Calculator::margin( $product->cost_price, $line_price, $pricelist->currency ) ) }}</td>
                <td>
