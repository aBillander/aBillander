...
@if ( $supplier->blocked )
                <span class="alert alert-danger" style="margin-right: 72px;">{{l('This Supplier is BLOCKED')}}</span>
...
@else
                ...
                <a href="{ { route('supplier.invoiceable.shippingslips', [$supplier->id]) }}" class="btn btn-navy" style="margin-right: 72px;"><i class="fa fa-object-group"></i> {{l('Group Shipping Slips')}}</a>
...
--}}
            ...
            <h2><a href="{{ URL::to('suppliers') }}">{{ l('Suppliers') }}</a> <span style="color: #cccccc;">/</span> {{ $supplier->name_regular }}</h2>
        </div>
    </div>
</div>
...
--}}
      ...
      <div class="col-lg-10 col-md-10 col-sm-9">
         ...
         {!! Form::model($supplier, array('route' => array('suppliers.update', $supplier->id), 'method' => 'PUT', 'class' => 'form')) !!}
          ...
          @include('suppliers._panel_main_data')

          @include('suppliers._panel_commercial')
...
{{--
          ...
          @include('suppliers._panel_pricerules')

          @include('suppliers._panel_statistics')
--}}
